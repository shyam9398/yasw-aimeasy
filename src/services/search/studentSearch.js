...
export async function searchStudentContent(query) {
  ...
  customSubjects.forEach((s) => {
    if ((!activeBranch || isSameBranch(s.branch, activeBranch)) && matches(q, s.name, s.code, s.branch)) {
      results.push({ type: 'subject', label: s.name, sub: 'Custom subject', action: { kind: 'legacySubject', raw: s } });
    }
  });
