...
import { fetchProfileByAuthId } from './profileService.js';
...
export function AuthProvider({ children }) {
  ...
  const refreshProfile = React.useCallback(async (nextSession = null) => {
    ...
    try {
      const { profile: row, error } = await withAuthTimeout(
        fetchProfileByAuthId(user.id),
        'fetchProfileByAuthId',
      );
      if (error) {
        console.error("PROFILE FETCH ERROR", error);
        console.warn('[AUTH] Profile load failed', { userId: user.id, error: error.message });
        return null;
      }
