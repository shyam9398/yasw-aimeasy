...
export function installCriticalFixes() {
  ...
  function hasActiveLegacyAdminSession() {
    ...
    if (/^\/(admin|subadmin|creator)(\/|$)/.test(route) && window.APP?.adminType) return true;
    try {
      const adminSession = JSON.parse(localStorage.getItem('edusync_admin_session') || 'null');
      return Boolean(adminSession?.type);
    } catch {
      return false;
    }
  }
  ...
  function syncGoogleAuthScreen() {
    const role = normalizeRole(window.APP?.role || getStoredLoginPortal()) || ROLE.STUDENT;
    ...
    if (titleEl) titleEl.textContent = isCreator ? 'Sign in as Teacher' : 'Sign in as Student';
    if (subEl) {
      subEl.textContent = isCreator
    ...
    if (roleTagEl) {
      roleTagEl.textContent = isCreator ? 'Teacher Login' : 'Student Login';
  ...
  if (window.googleSignIn) {
    window.googleSignIn = function googleSignInStudentOnly(accountType) {
      ...
      const selectedRole = normalizeRole(window.APP?.role || getStoredLoginPortal()) || ROLE.STUDENT;
  ...
  window.submitProfile = async function submitProfileDbBacked() {
    ...
    try {
      ...
      if (window.APP.user.googleId) {
        localStorage.setItem('edusync_user_' + window.APP.user.googleId, JSON.stringify(window.APP.user));
      }
      ...
      if (window.APP.user.googleId || window.APP.user.id) {
        localStorage.setItem('edusync_user_' + (window.APP.user.googleId || window.APP.user.id), JSON.stringify(window.APP.user));
      }
...
if (!/^[0-9]{10}$/.test(phone)) {
  ...
  window.updateLandingStats = async function updateLandingStatsLive() {
    ...
    setText('stat-students', stats.students);
  ...
  window.aimeasyDeleteContent = deleteContentItem;
  window.aimeasyUpdateContent = updateContentItem;
  window.aimeasyNormalizeContentType = normalizeContentType;
  window.aimeasySaveUnitRoadmap = saveUnitRoadmap;
  window.aimeasyFetchUnitRoadmap = fetchUnitRoadmap;
  window.aimeasySaveLinkedContentItem = saveLinkedContentItem;
  ...
  window.__AIMEASY_SUPABASE__?.channel?.('curriculum-dashboard')
    ?.on('postgres_changes', { event: '*', schema: 'public', table: 'subjects' }, () => window.renderSubAdminDashboardLive?.())
    ?.on('postgres_changes', { event: '*', schema: 'public', table: 'units' }, () => window.renderSubAdminDashboardLive?.())
    ?.on('postgres_changes', { event: '*', schema: 'public', table: 'content_items' }, () => window.renderSubAdminDashboardLive?.())
    ?.subscribe?.();
